...
class Loader(object):
    ...
    def _imread(self, file):
        # imageio first (skimage ≥0.25 deprecates the `plugin=` argument on imread).
        try:
            import imageio.v3 as iio

            arr = np.asarray(iio.imread(file))
        except Exception:
            arr = skimage_io.imread(file, as_gray=self.as_gray)
        if arr is None:
            return arr
        if self.as_gray:
            if arr.ndim == 3:
...
class BooneTransform(object):
    ...
    def __call__(self, img_small):
        try:
            ...
            m = footprint_rectangle((self.square_size, self.square_size))
        except Exception:
            m = morphology.square(self.square_size)
        try:
            img_th = morphology.black_tophat(img_small, footprint=m)
        except TypeError:
            img_th = morphology.black_tophat(img_small, selem=m)
        img_sob = abs(filters.sobel_v(img_th))
        try:
            img_closed = morphology.closing(img_sob, footprint=m)
        except TypeError:
            img_closed = morphology.closing(img_sob, selem=m)
